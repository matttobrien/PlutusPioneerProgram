...
mkPolicy pkh deadline () ctx = traceIfFalse "you are not the owner!" (txSignedBy info $ unPaymentPubKeyHash pkh) &&
                               traceIfFalse "the deadline has passed!" (to deadline `contains` validRange)
    where
