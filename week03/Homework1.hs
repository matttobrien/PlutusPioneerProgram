...
mkValidator dat () ctx
    -- check if the PaymentPubKeyHash of a beneficiary is provided with the transaction & check if the provided transaction range mataches one of the two cases
    | (unPaymentPubKeyHash (beneficiary1 dat) `elem` signatures) && (to       (deadline dat) `contains` validRange) = True
    | (unPaymentPubKeyHash (beneficiary2 dat) `elem` signatures) && (from (1 + deadline dat) `contains` validRange) = True
    | otherwise = False
  where
