...
-- Finished represents the final state of the state machine
...
transition game s r = case (stateValue s, stateData s, r) of
    -- no need to check for the NFT, it is taken care of by the state machine
...
-- init state machine using plutus state machine lib
...
firstGame fp = do
    ...
    -- tell other player what the valid token (NFT) is
    tell $ Last $ Just tt

    -- wait for other player to send UTXO
    waitUntilTimeHasPassed $ fpPlayDeadline fp

    -- grab UTXO from the state machine
    m <- mapError' $ getOnChainState client
    case m of
        Nothing     -> throwError "game output not found"
        Just (o, _) -> case tyTxOutData $ ocsTxOut o of
            -- runStep runs the next step in the state machine using the user input
